...
export function buildLlmsTxtBody(): string {
  const lines: string[] = [
    ...
    "- **Notary**: Remote online notarization (RON), on-demand / same-day when capacity allows, loan signings, mobile notary, estate and employer packages. State hubs at /notary/locations with examples like /notary/utah/lehi/ and /notary/california/los-angeles/.",
...
export function buildLlmsFullTxtBody(): string {
  const lines: string[] = [
    ...
    "",
    "### Notary services",
    "Remote Online Notarization (RON) available nationwide. On-demand scheduling — we try to accommodate same-day when capacity allows (best-effort, not a guaranteed SLA). Mobile notary across the Wasatch Front. Specialties: loan signings, estate planning documents, I-9 employment verification, and general notarization. Employer packages for bulk signings. Locations hub highlights state RON coverage and Utah city pages.",
    ...
    "A: Yes. Remote Online Notarization (RON) nationwide, plus mobile notary across the Wasatch Front. On-demand scheduling — we try to accommodate same-day when capacity allows. Loan signings, estate docs, I-9, and general notarization. See /notary/on-demand.",
    "",
